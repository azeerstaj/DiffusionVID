...
at::Tensor nms_cuda(const at::Tensor boxes, float nms_overlap_thresh) {
  ...
  const float* boxes_dev = boxes_sorted.contiguous().data_ptr<float>();

  unsigned long long* mask_dev = nullptr;
  auto mask_size = boxes_num * col_blocks * sizeof(unsigned long long);
  cudaError_t err = cudaMalloc(&mask_dev, mask_size);
  TORCH_CHECK(err == cudaSuccess, "CUDA malloc failed for mask");
  ...
  TORCH_CHECK(err == cudaSuccess, "nms_kernel launch failed");

  std::vector<unsigned long long> mask_host(boxes_num * col_blocks);
  err = cudaMemcpy(mask_host.data(), mask_dev, mask_size, cudaMemcpyDeviceToHost);
  TORCH_CHECK(err == cudaSuccess, "cudaMemcpy failed for mask");
  ...
  return std::get<0>(
    order_t.index({
      keep.narrow(0, 0, num_to_keep).to(order_t.device(), order_t.scalar_type())
    }).sort(0, false)
  );
}
