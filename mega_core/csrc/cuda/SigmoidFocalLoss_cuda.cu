...
// Modified for modern PyTorch and CUDA compatibility
...
// CUDA kernel loop macro (unchanged)
...
at::Tensor SigmoidFocalLoss_forward_cuda(
  ...
  dim3 grid(std::min(ceil_div(losses_size, 512), 4096));
  dim3 block(512);
...
at::Tensor SigmoidFocalLoss_backward_cuda(
  ...
  dim3 grid(std::min(ceil_div(d_logits_size, 512), 4096));
  dim3 block(512);
