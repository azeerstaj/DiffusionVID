...
std::tuple<at::Tensor, at::Tensor> ROIPool_forward_cuda(
  ...
  auto argmax = at::zeros({num_rois, channels, pooled_height, pooled_width},
...
at::Tensor ROIPool_backward_cuda(
  ...
  auto grad_input = at::zeros({batch_size, channels, height, width},
                              grad.options());

  cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  dim3 block(512);
  dim3 grid(std::min(CeilDiv(static_cast<int>(grad.numel()), 512), 4096));
